...
class TestFirebaseVerifyEndpoint:
    ...
    def test_invalid_token_returns_401(self, api_client):
        ...
        with patch(
            "firebase_admin.auth.verify_id_token", side_effect=firebase_auth.InvalidIdTokenError("bad token", None)
        ):
            response = api_client.post(FIREBASE_VERIFY_URL, {"id_token": "garbage"}, format="json")
    ...
    def test_expired_token_returns_401(self, api_client):
        ...
        with patch(
            "firebase_admin.auth.verify_id_token", side_effect=firebase_auth.ExpiredIdTokenError("expired", None)
        ):
            response = api_client.post(FIREBASE_VERIFY_URL, {"id_token": "expired_token"}, format="json")
        assert response.status_code in (401, 403)
    ...
    def test_revoked_token_returns_401(self, api_client):
        ...
        with patch("firebase_admin.auth.verify_id_token", side_effect=firebase_auth.RevokedIdTokenError("revoked")):
            response = api_client.post(FIREBASE_VERIFY_URL, {"id_token": "revoked_token"}, format="json")
        assert response.status_code in (401, 403)
    ...
    def test_unregistered_phone_returns_404(self, api_client):
        ...
        with patch("firebase_admin.auth.verify_id_token", return_value=make_mock_decoded_token(phone="+919111111111")):
            response = api_client.post(FIREBASE_VERIFY_URL, {"id_token": "valid_but_unknown_phone"}, format="json")
    ...
    def test_inactive_worker_returns_404(self, api_client, inactive_worker_registration):
        ...
        with patch("firebase_admin.auth.verify_id_token", return_value=make_mock_decoded_token(phone="+919000000000")):
            response = api_client.post(FIREBASE_VERIFY_URL, {"id_token": "valid_token_inactive_worker"}, format="json")
        assert response.status_code == 404
    ...
    def test_successful_login_returns_tokens(self, api_client, active_worker_registration):
        ...
        with patch("firebase_admin.auth.verify_id_token", return_value=make_mock_decoded_token(phone="+919876543210")):
            response = api_client.post(FIREBASE_VERIFY_URL, {"id_token": "valid_token"}, format="json")
    ...
    def test_phone_normalization_prefixed_to_10digit(self, api_client, tendigit_worker_registration):
        ...
        with patch("firebase_admin.auth.verify_id_token", return_value=make_mock_decoded_token(phone="+919988776655")):
            response = api_client.post(FIREBASE_VERIFY_URL, {"id_token": "valid_token_prefixed"}, format="json")
        assert response.status_code == 200
    ...
    def test_phone_normalization_10digit_to_prefixed(self, api_client, active_worker_registration):
        ...
        with patch("firebase_admin.auth.verify_id_token", return_value=make_mock_decoded_token(phone="9876543210")):
            response = api_client.post(FIREBASE_VERIFY_URL, {"id_token": "valid_token_10digit"}, format="json")
        assert response.status_code == 200
    ...
    def test_no_phone_in_token_returns_400(self, api_client):
        ...
        with patch("firebase_admin.auth.verify_id_token", return_value={"uid": "uid123"}):
            response = api_client.post(FIREBASE_VERIFY_URL, {"id_token": "valid_token_no_phone"}, format="json")
        assert response.status_code == 400
    ...
    def test_firebase_service_unavailable_returns_503(self, api_client):
        ...
        with patch(
            ...
            response = api_client.post(FIREBASE_VERIFY_URL, {"id_token": "some_token"}, format="json")
