...
describe("user input storage keys", () => {
  it("auth session keys are stable", () => {
    expect(AUTH_USER_KEY).toBe("shaasthi_auth_user_json");
    expect(AUTH_WORKER_KEY).toBe("shaasthi_auth_worker_json");
    expect(AUTH_PENDING_PHONE_KEY).toBe("shaasthi_auth_pending_phone");
    expect(AUTH_PENDING_LOCALE_KEY).toBe("shaasthi_auth_pending_locale");
  });

  it("survey draft key is per-patient", () => {
    expect(draftKey("p1")).toBe("shaasthi_survey_draft_p1");
  });
});
