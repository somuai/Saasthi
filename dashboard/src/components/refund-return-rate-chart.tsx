...
export function RefundReturnRateChart() {
	...
	const lastW = referralsDaily7.at(-1) ?? first;
		...
		<Card className="md:col-span-2 border-slate-200/60 shadow-none">
			...
			<CardFooter className="border-t border-slate-100 pt-3">
				...
				<Button
					...
					size="xs"
					variant="ghost"
					render={<a href="#/referrals" />}
					nativeButton={false}
				>
					Referrals list
					<ArrowRightIcon aria-hidden="true" data-icon="inline-end" className="h-3 w-3 ml-1" />
				</Button>
			</CardFooter>
		</Card>
	);
